...
public class KeyBindingsTest implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		KeyBinding stickyBinding1 = KeyBindingHelper.registerStickyKeyBinding("key.fabric-key-binding-api-v1-testmod.test_keybinding_sticky_1", GLFW.GLFW_KEY_R, "key.category.first.test", StickyKeybindingDefaultBehavior.HOLD);
		KeyBinding stickyBinding2 = KeyBindingHelper.registerStickyKeyBinding("key.fabric-key-binding-api-v1-testmod.test_keybinding_sticky_2", GLFW.GLFW_KEY_G, "key.category.first.test", StickyKeybindingDefaultBehavior.TOGGLE);
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			if (stickyBinding1.isPressed()) {
				client.player.sendMessage(Text.literal("Sticky Key 1 was pressed!"), false);
			}

			if (stickyBinding2.isPressed()) {
				client.player.sendMessage(Text.literal("Sticky Key 2 was pressed!"), false);
			}
